...
def run_search_movie(scraper, codes: list[str], kind: int) -> dict:
    ...
    with scraper.store._conn() as conn:
        conn.execute("INSERT OR IGNORE INTO list_sources (list_code, list_path, list_params, max_pages, last_scanned_page, created_at) VALUES ('TOP250', '/top250', 'f=all', 100, 0, datetime('now'))")
        conn.commit()
    for i, code in enumerate(codes, 1):
        try:
            ...
            with scraper.store._conn() as conn:
                conn.execute(
                    "INSERT OR IGNORE INTO list_sources (list_code, list_path, list_params, max_pages, last_scanned_page, created_at) VALUES ('TOP250', '/top250', 'f=all', 100, 0, datetime('now'))")
            ...
            try:
                if args.command == "scan":
                    ...
                    logger.info(f"执行演员爬取: {actor_url}" + (f"（actor_id={_actor_id}）" if _actor_id else "")
                                ...
                                + ("（仅单体作品）" if _solo else ""))
                    result = a.crawl_actor_full(actor_url, actor_id=_actor_id, max_co_star=_max_co, solo_only=_solo)
