...
export const api = {
  ...
  tasks: {
    ...
    batchForceVisit: (task_ids: number[]) =>
      http.post<{ ok: boolean; updated: number }>('/api/tasks/batch/force-visit', task_ids).then((r) => r.data),
    retryNow: () =>
  ...
  actors: {
    ...
    crawlWorks: (actorId: number, maxCoStar = 0, soloOnly = false, videoFilter: string = 'none', excludeVr = false, since = '') =>
      http.post<{ ok: boolean; pid: number; mode: string; actor_url: string }>(`/api/actors/${actorId}/crawl-works`, { max_co_star: maxCoStar, solo_only: soloOnly, video_filter: videoFilter, exclude_vr: excludeVr, since }).then((r) => r.data),
  ...
  downloaders: {
    ...
    testConnection: (downloader: string, save_path?: string) =>
      http.post<ApiOk & { msg?: string }>('/api/downloaders/test-connection', { downloader, save_path }).then((r) => r.data),
    xunleiMcpPilot: () =>
      http.post<{ ok: boolean; server?: unknown; tools?: { name: string; description: string }[]; message?: string }>(
        '/api/downloaders/xunlei-mcp-pilot'
      ).then((r) => r.data),
    logs: (limit = 100) =>
  ...
  subscriptions: {
    ...
    fillAllWorks: (waitLimitMin: number, maxCoStar = 0, since = '') =>
      http.post<{ ok: boolean; message: string }>('/api/subscriptions/fill-all-works', { wait_limit_min: waitLimitMin, max_co_star: maxCoStar, since }).then((r) => r.data),
    /** 查询「全部补齐作品」任务进度 */
    fillWorksStatus: () =>
